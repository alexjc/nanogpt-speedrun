...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, block_mask: BlockMask):
        ...
        if ve is not None:
            ...
            v = self.lambdas[0] * v
        y = flex_attention(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2), block_mask=block_mask, scale=self.attn_scale, kernel_options=flex_kernel_options).transpose(1, 2)
...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor, num_layers: int = None):
        ...
        loss = F.cross_entropy(logits.view(-1, logits.size(-1)), target_seq, reduction='none')
        return loss
...
class DistributedDataGenerator:
    ...
    def __init__(self, filename_pattern: str, batch_size: int, rank : int, world_size : int):
        ...
        self.replay_start = 125 # collects data from 0.5x, and replays from 1.5x onwards
        self.replay_chunk_count = 12
        self.replay_chunk_size = 192

        # Runtime variables.
        self.replay_chunks = []
        self.replay_iter = 0
        self.replay_backlog = 0

        # Initialize queues and threads.
        self.fetch_queue = queue.Queue(maxsize=2)
        self.fetch_thread = threading.Thread(target=self._do_fetch_data, daemon=True)
        self.fetch_thread.start()

        self.feedback_queue = queue.Queue(maxsize=1)
        self.feedback_stats = torch.zeros((50257,), dtype=torch.long, device='cpu')
        self.feedback_thread = threading.Thread(target=self._do_feedback_loss, daemon=True)
    ...
    def _do_fetch_data(self):
        ...
        while True:
            ...
            if self.replay_iter > self.replay_start * 1.5:
                doc_boundaries = (buf == 50256).nonzero().squeeze(1).tolist()

                if len(doc_boundaries) < self.replay_chunk_count:
                    self.replay_backlog += self.replay_chunk_count
                    replay_count = self.replay_chunk_count * 2
                else:
                    replay_count = self.replay_chunk_count

                # Calculate start, end and length of each document
                doc_starts = [-1] + doc_boundaries
                doc_ends = doc_boundaries + [len(buf)]
                doc_lengths = [end - start - 1 for start, end in zip(doc_starts, doc_ends)]

                for _ in range(replay_count):
                    chunk = self.replay_chunks.pop(0)

                    did = max(range(len(doc_lengths)), key=lambda i: doc_lengths[i])
                    doc_end = doc_ends[did]
                    assert doc_end == len(buf) or buf[doc_end] == 50256
                    assert len(chunk) < doc_lengths[did]

                    buf[doc_end - len(chunk):doc_end] = chunk
                    buf[doc_end - len(chunk) - 1] = 50256

                    doc_ends[did] -= len(chunk) + 1
                    doc_lengths[did] -= len(chunk) + 1
    ...
    def feedback(self, tokens, losses):
        assert tokens.shape[0] == losses.shape[0]
        self.replay_iter += 1
        random.shuffle(self.replay_chunks)
        self.feedback_queue.put((tokens.cpu(), losses.cpu()))

    @torch.no_grad()
    def _do_feedback_loss(self):
        while True:
            tokens, losses = self.feedback_queue.get()
            doc_boundaries = [-1] + (tokens == 50256).nonzero().squeeze(1).tolist() + [len(tokens)]

            self.feedback_stats += torch.bincount(tokens.flatten(), minlength=50257)
            ...
            for start, end in zip(doc_boundaries[:-1], doc_boundaries[1:]):
                ...
                cur = start
                while cur + self.replay_chunk_size < end - 8:
                    cur += self.replay_chunk_size
            ...
            assert sum(b<0 for b in doc_boundaries) >= self.replay_chunk_count
            valid_chunks = sum(1 for i in range(len(doc_boundaries)-1) if (doc_start[i]>=0) and (doc_end[i]<0))
            if valid_chunks < self.replay_chunk_count * 2:
                self.replay_backlog += self.replay_chunk_count
                assert self.replay_backlog < self.replay_chunk_count * 10
                continue

            # Calculate average cross entropy, multiplied by inverse token logfreq, for each chunk.
            ...
            top_indices = sorted(range(len(doc_losses)), key=lambda i: doc_losses[i] * (doc_start[i]>=0) * (doc_end[i]<0), reverse=True)

            k = -(-self.replay_backlog // self.replay_chunk_count) # math.ceil
            count = self.replay_chunk_count + k
            self.replay_backlog -= k

            # Extract tokens for each of the worst docs
            for j in top_indices[:count]:
                worst_tokens = tokens[abs(doc_start[j])+1:abs(doc_end[j])]
                assert doc_start[j] == -1 or abs(doc_end[j]) - abs(doc_start[j]) == self.replay_chunk_size
...
for _ in range(warmup_steps):
    ...
    model(inputs.to(torch.int32), targets, get_window_size_blocks(0)).mean().backward()
    for param in model.parameters():
...
for step in range(train_steps + 1):
    ...
    loss.mean().backward()
    train_loader.feedback(inputs, loss)
    del loss
