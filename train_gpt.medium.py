...
class Muon(torch.optim.Optimizer):
    ...
    def step(self):
        for group in self.param_groups:
            ...
            params_used_only = [p for p in params if p.grad is not None]
            for base_i in range(len(params_used_only))[::self.world_size]:
                if base_i + self.rank < len(params_used_only):
                    p = params_used_only[base_i + self.rank]
                    g = p.grad
                    state = self.state[p]
                ...
                params_world = params_used_only[base_i : base_i + self.world_size]
            update_prev()
...
class Rotary(nn.Module):
    def __init__(self, dim: int, max_seq_len=65536 * 4):
        super().__init__()
...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor, num_layers: int = None):
        num_layers = num_layers or (self.num_encoder_layers + self.num_decoder_layers) // 2
        BLOCK_SIZE = 128
        ...
        block_masks.reverse()
        for i in range(self.num_decoder_layers - num_layers, self.num_decoder_layers):
...
dist.init_process_group(backend="nccl", device_id=device) # , timeout=timedelta(seconds=5))
dist.barrier()
...
# load data
train_loader = DistributedDataGenerator(args.train_files, args.batch_size, rank, world_size)
train_loader_iter = iter(train_loader)

model = GPT(vocab_size=28416, num_layers=16, num_heads=8, model_dim=1024).cuda()
...
optimizer2 = Muon(hidden_matrix_params, lr=0.025*k, momentum=0.95, rank=rank, world_size=world_size)
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        val_loader = iter(DistributedDataGenerator(args.val_files, val_bs, rank, world_size))
        val_loss = 0
        with torch.no_grad():
